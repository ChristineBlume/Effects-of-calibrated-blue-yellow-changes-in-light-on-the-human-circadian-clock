function [data_reref] = preprocessing_Twilight_SLEEP (path_data, computer)
...
%         load(strcat(VPname, '_cfg_artrej'), 'cfg_artrej_contdata');  %% load cfg output from ft_databrowser
...
%         end
        ...
        data_cont_segm_finalclean = ft_selectdata(cfg, data_cont_segm); % only segmented between lights off/on; no artifact rejection
        data_SWAsegm_finalclean = ft_selectdata(cfg, data_SWAsegm_clean); % segmentation into 30s epochs
